...
public class Storage {
	...
	public static void saveObject(String key, Object obj) {
		try {
			...
			String base64 = new String(Base64.encode(json.getBytes(),
					Base64.DEFAULT));
	...
	public static <T> T getObject(String key, Type type) {
		try {
			FileInputStream fileInputStream = new FileInputStream(MyApplication
	...
	public static void clearObject(String key) {
		...
		FileUtil.delete(new File(MyApplication.getInstance().getCacheDir()
				.getPath()
				+ File.separator + key));
	}
