...
public class SimpleTableView extends TableLayout {// 一个用于显示简易表格的VIEW

	protected int m_ColumnN = 2;// 列的数目。该值只能在构造函数中设置，设置之后不能修改。

	int m_LineColor = Color.GRAY;// 线的颜色
	...
	public int AddRow(Object objects[])// 添加一行，返回行数。如果objects的数目小于m_ColumnN则返回0。
	{
		if (objects == null)
			return 0;
		if (objects.length < m_ColumnN)
			return 0;
	...
	public View GetCellView(int row, int column)// 获得某一个单元格的View，row为行数，column为列数，从0开始
	{
		if (row < 0 || row >= m_Rows.size())
			...
			if (column < 0 || column >= m_Views.get(row).size())
				return null;
			else
				return m_Views.get(row).get(column);
		}
	}
